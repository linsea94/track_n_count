...
def run(
    ...
    global frame_cnt, count1, count2, fps
    source = str(source)
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                LOGGER.info(f'{s}\nN-S way count :{count1}, E-W way count : {count2}, ')
                LOGGER.info(f'Frame: {frame_cnt} Time: {sec} sec been finished.')
                LOGGER.info(f'YOLO:({t3 - t2:.3f}s), StrongSORT:({t5 - t4:.3f}s)\n\n')
            ...
            # Stream results
            im0 = annotator.result() #drow video result
            if show_vid:
                
                cv2.line(im0, p1, p2, color=(255, 255, 0), thickness = 2)
...
def parse_opt():
    ...
    # class 0 is person, 1 is bycicle, 2 is car, 3 is motorcycle, 4 is airplane
    # 5 is bus, 6 is train, 7 is truck, 9 is traffic light
